...
export default function ProductModal({ product, onClose }) {
  ...
  const lbSrc       = allImages[lbIdx] ?? null
  ...
  const lbPrev = () => { setLbLoaded(false); setLbIdx(i => (i - 1 + allImages.length) % allImages.length) }
  const lbNext = () => { setLbLoaded(false); setLbIdx(i => (i + 1) % allImages.length) }

  const openLightbox = () => { setLbIdx(idx); setLbLoaded(false); setLightboxOpen(true) }
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (lightboxOpen) {
        if (e.key === 'ArrowLeft'  && hasMultiple) lbPrev()
        if (e.key === 'ArrowRight' && hasMultiple) lbNext()
      } else {
  ...
  }, [onClose, hasMultiple, idx, lightboxOpen, lbIdx])
    ...
    <>
          ...
          >
            ...
            {currentSrc && !imgError ? (
              ...
              >
                ...
                <div style={{
                  ...
                  opacity: imgHovered && imgLoaded ? 1 : 0,
            ...
            {hasMultiple && (
              <>
                <button onClick={e => { e.stopPropagation(); prev() }} style={{
                  ...
                  WebkitBackdropFilter: 'blur(4px)', zIndex: 4,
                }}>
            ...
            {hasMultiple && (
              <div style={{
                ...
                display: 'flex', justifyContent: 'center', gap: '0.375rem', zIndex: 4,
              }}>
                {allImages.map((_, i) => (
                  <button
                    key={i}
                    onClick={e => { e.stopPropagation(); setImgLoaded(false); setImgError(false); setIdx(i) }}
                    style={{
      ...
      {lightboxOpen && (
        <div
          ...
          onTouchEnd={e => {
            if (lbTouchStartX.current === null || !hasMultiple) return
            const dx = e.changedTouches[0].clientX - lbTouchStartX.current
            if (dx > 45) lbPrev()
            else if (dx < -45) lbNext()
            ...
            <>
              <button
                ...
                style={{
                  ...
                  width: 44, height: 44, borderRadius: '50%',
                  background: 'rgba(6,12,26,0.8)', border: '1px solid rgba(92,200,224,0.3)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  color: '#5cc8e0', cursor: 'pointer',
          ...
          {hasMultiple && (
            <div style={{
              ...
              {allImages.map((_, i) => (
                <button
                  key={i}
                  onClick={e => { e.stopPropagation(); setLbLoaded(false); setLbIdx(i) }}
                  style={{
                    width: i === lbIdx ? 20 : 8, height: 8,
                    borderRadius: 4,
                    background: i === lbIdx ? '#5cc8e0' : 'rgba(255,255,255,0.25)',
                    border: 'none', cursor: 'pointer', padding: 0,
                    transition: 'all 0.2s',
                  }}
                />
